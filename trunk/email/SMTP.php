...
class SMTP
{
    ...
    public function sendEmail($to, $fromPerson, $fromEmail, $subject, $body, $isHtml = false)
    {
        $res = $this->connect();

        if ($res == false) {
            $this->lastError = "Connect error: " . $this->getLastError();
            $this->lastStatus = 'error_connect';
        ...
        if ($res == false) {
            ...
            $this->lastStatus = 'error_authenticate';
        ...
        if ($code != 250) {
            ...
            $this->lastStatus = 'error_mail_from';
        ...
        if ($code != 250) {
            ...
            $this->lastStatus = 'error_rcpt_to_command';
        ...
        if ($isHtml) {
            $htmlHeader = "Mime-Version: 1.0;{$this->newline}Content-Type: text/html; charset='ISO-8859-1';{$this->newline}Content-Transfer-Encoding: 7bit;{$this->newline}";
        }

        $response = $this->sendData("Subject: {$subject}{$this->newline}To:<{$to}>{$this->newline}From: {$fromPerson}<{$fromEmail}>{$this->newline}{$htmlHeader}{$this->newline}{$this->newline}{$body}{$this->newline}.");
        $code     = $this->responseCode($response);
    ...
    protected function connect()
    {
        ...
        while ($recconceted < $this->recconectTime) {
